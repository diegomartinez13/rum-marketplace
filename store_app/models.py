...
class Product(models.Model):
    ...
    discount = models.DecimalField(default=Decimal('0.00'), decimal_places=2, max_digits=6)
    business_vendor = models.ForeignKey('Business', on_delete=models.CASCADE, null=True, blank=True, related_name='products')
    user_vendor = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='products')
...
class ServiceCategory(models.Model):
    ...
    def __str__(self): return self.name
    class Meta: verbose_name_plural = "Service Categories"
...
class Service(models.Model):
    ...
    discount = models.DecimalField(default=Decimal('0.00'), decimal_places=2, max_digits=6)
    business_provider = models.ForeignKey('Business', on_delete=models.CASCADE, null=True, blank=True, related_name='services')
    user_provider = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='services')
...
class ProductOrder(models.Model):
    ...
    def __str__(self): return f"{self.product} ordered by {self.user}"
...
class ServiceRequest(models.Model):
    ...
    def __str__(self): return f"{self.service} requested by {self.user}"
    class Meta: verbose_name_plural = "Service Requests"
...
class Business(models.Model):
    ...
    business_description = models.TextField(max_length=250, default='', blank=True, null=True)
    business_logo = models.ImageField(upload_to="uploads/businesses/logos/")

    def __str__(self): return self.business_name
    class Meta: verbose_name_plural = "Businesses"

class BusinessCategory(models.Model):
    name = models.CharField(max_length=50)
    def __str__(self): return self.name
    class Meta: verbose_name_plural = "Business Categories"

class Inventory(models.Model):
    business = models.ForeignKey(Business, on_delete=models.CASCADE)
    def __str__(self): return f"Inventory of {self.business.business_name}"
...
class StockedProduct(models.Model):
    ...
    def __str__(self): return f"{self.quantity} of {self.product.name} in {self.inventory.business.business_name}"
    class Meta: verbose_name_plural = "Stocked Products"
...
class StockedService(models.Model):
    ...
    def __str__(self): return f"{self.service.name} in {self.inventory.business.business_name}"
    class Meta: verbose_name_plural = "Stocked Services"
...
class Review(models.Model):
    ...
    date = models.DateField(auto_now_add=True)
    def __str__(self):
        target = self.product.name if self.product else self.service.name if self.service else "Unknown"
...
class Cart(models.Model):
    ...
    subtotal = models.DecimalField(default=Decimal('0.00'), decimal_places=2, max_digits=10)
    def __str__(self): return f"Cart of {self.user.id} as of {self.last_updated}"
